...
def deploy(img_path):
    Model_job = '/dli/data/digits/20190615-173447-232c'
    caffe.set_mode_gpu()
    
    # Initialize the Caffe model using the model trained in DIGITS. Which two files constitute your trained model?
    net = caffe.Classifier(Model_job+'/deploy.prototxt', Model_job+'/snapshot_iter_1026.caffemodel',
                           channel_swap=(2,1,0),
                           raw_scale=255,
                           image_dims=(256, 256))
    ...
    Dataset = '/dli/data/digits/20190615-173238-e759'
    input_image= caffe.io.load_image(img_path)
    input_image = cv2.resize(input_image, (256,256))
    mean_image = caffe.io.load_image(Dataset+'/mean.jpg')
    ...
    if prediction[0].argmax() == 0 :##REPLACE WITH THE CONDITION THAT WOULD MAKE THE FUNCTION RETURN WHALE##:
        return "whale"
    else:
        return "not whale"
